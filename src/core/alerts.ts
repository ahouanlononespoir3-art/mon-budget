...
export function buildBudgetAlerts(input: { calculation: BudgetCalculation; month: BudgetMonth; settings: BudgetSettings; expenses: Expense[]; plannedExpenses: PlannedExpense[]; recurringExpenses: RecurringExpense[]; goals: SavingsGoal[]; futurePurchases: FuturePurchase[]; today?: Date }): BudgetAlert[] {
  ...
  if (settings.alertOverspendingRisk && calculation.forecastEndBalance < month.minimumEndBalance) alerts.push({ id: "reserve-risk", severity: "critical", title: "Réserve menacée", message: `Le solde prévu est de ${formatMoney(Math.max(0, calculation.forecastEndBalance), month.currency)}, sous la réserve minimale.` });
  ...
  if (settings.alertBelowPlan && variance > 0) alerts.push({ id: "planned-variance", severity: "warning", title: "Dépenses supérieures au prévu", message: `Les écarts défavorables représentent ${formatMoney(variance, month.currency)}.` });
