...
export function Notifications() {
  ...
  return <div className="space-y-6"><header><p className="text-sm font-medium text-slate-500">Centre de notifications</p><h1 className="mt-1 text-2xl font-bold text-slate-900">Notifications</h1><p className="mt-1 text-sm text-slate-500">Les événements importants de ton budget sont regroupés ici.</p></header><Card>{notifications.length === 0 ? <div className="flex flex-col items-center py-10 text-center"><CheckCircle2 className="text-emerald-500" size={36} /><h2 className="mt-3 font-semibold">Tout est calme</h2><p className="mt-1 text-sm text-slate-500">Aucune notification urgente pour le moment.</p></div> : <div className="space-y-3">{notifications.map((notification) => { const Icon = notification.severity === "info" ? Info : AlertTriangle; return <article key={notification.id} className="flex gap-3 rounded-xl border border-slate-200 p-4"><Icon className={notification.severity === "critical" ? "text-red-600" : notification.severity === "warning" ? "text-amber-600" : "text-blue-600"} /><div><h2 className="font-semibold text-slate-900">{notification.title}</h2><p className="mt-1 text-sm text-slate-600">{notification.message}</p></div><Bell className="ml-auto shrink-0 text-slate-300" size={16} /></article>; })}</div>}</Card></div>;
}
