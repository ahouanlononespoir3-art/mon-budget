...
export function Assistant() {
  ...
  return <div className="space-y-6"><header><p className="text-sm font-medium text-slate-500">Analyse personnelle</p><h1 className="mt-1 text-2xl font-bold text-slate-900">Assistant budgétaire</h1><p className="mt-1 text-sm text-slate-500">Des recommandations calculées à partir de tes données.</p></header><Card><div className="flex items-start gap-3"><BrainCircuit className="mt-1 text-blue-600" /><div><h2 className="font-semibold">Lecture du mois</h2><p className="mt-1 text-sm text-slate-500">Prévision : {formatMoney(calculation.forecastEndBalance)} en fin de mois · Risque : {calculation.riskLevel}</p></div></div></Card><div className="grid gap-4 md:grid-cols-2">{advice.map((item) => <Card key={item}><div className="flex gap-3"><Lightbulb className="shrink-0 text-amber-500" size={20} /><p className="text-sm leading-6 text-slate-700">{item}</p></div></Card>)}</div><Card title="Scénarios de fin de mois"><div className="grid gap-3 md:grid-cols-3">{scenarios.map((scenario) => <div key={scenario.name} className="rounded-xl bg-slate-50 p-4"><p className="font-semibold capitalize">{scenario.name}</p><p className="mt-2 text-lg font-bold">{formatMoney(scenario.projectedEndBalance)}</p><p className="mt-1 text-xs text-slate-500">{scenario.description}</p></div>)}</div></Card><Card><div className="flex gap-3"><ShieldAlert className="text-blue-600" size={20} /><p className="text-sm text-slate-600">La limite sécurisée tient compte de la réserve minimale ; les dépenses planifiées restent à honorer avant les achats facultatifs.</p></div></Card></div>;
}
