...
export function Alerts() {
  ...
  return <div className="space-y-6"><header><p className="text-sm font-medium text-slate-500">Surveillance intelligente</p><h1 className="mt-1 text-2xl font-bold text-slate-900">Alertes</h1><p className="mt-1 text-sm text-slate-500">Les signaux sont recalculés à partir de tes opérations actuelles.</p></header><Card>{alerts.length === 0 ? <div className="flex items-center gap-3 text-emerald-700"><CheckCircle2 /><p>Aucune alerte active.</p></div> : <div className="space-y-3">{alerts.map((alert) => <div key={alert.id} className={`flex items-start gap-3 rounded-xl border p-4 ${alert.severity === "critical" ? "border-red-200 bg-red-50 text-red-900" : alert.severity === "warning" ? "border-amber-200 bg-amber-50 text-amber-900" : "border-blue-200 bg-blue-50 text-blue-900"}`}>{alert.severity === "info" ? <Info className="shrink-0" size={20} /> : <AlertTriangle className="shrink-0" size={20} />}<div><p className="font-semibold">{alert.title}</p><p className="mt-1 text-sm">{alert.message}</p></div></div>)}</div>}</Card><Card title="Règles surveillées"><div className="space-y-3 text-sm text-slate-600"><p><Bell className="mr-2 inline" size={16} />Seuil de budget, réserve et limite quotidienne</p><p><Bell className="mr-2 inline" size={16} />Écarts prévu/réel et moyenne hebdomadaire</p><p><Bell className="mr-2 inline" size={16} />Factures récurrentes, objectifs et achats futurs</p></div></Card></div>;
}
